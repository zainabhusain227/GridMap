...
public class UAP_AccessibilityManager : MonoBehaviour
{
	...
	//[Header("Extras")]
	public Transform customAudioSourceLocation;
	void ReadType()
	{
		if (m_CurrentItem == null)
			return;

		Vector3 audioPlayerPos = m_CurrentItem.m_Object.transform.position;
		Debug.Log(audioPlayerPos.ToString());

		if (customAudioSourceLocation == null) {
			customAudioSourceLocation = GameObject.Find("Custom TTS").transform;
			customAudioSourceLocation.GetComponent<AudioSource>().spatialBlend = 1;
			customAudioSourceLocation.GetComponent<AudioSource>().minDistance = 500;
            customAudioSourceLocation.GetComponent<AudioSource>().maxDistance = 1000;
            customAudioSourceLocation.GetComponent<AudioSource>().pitch = 1f;

		}
			

		customAudioSourceLocation.position = audioPlayerPos;
	...
	public void customIncrementContainter() {
	...
	public void customDecrementContainter()
	{
	...
	public void CustomSelect2(GameObject input) {
		//m_CurrentItem = input.GetComponent<>()
		InteractWithElement(m_CurrentItem);
		ReadItem(m_CurrentItem);

	}

	public void CustomTurnOnAccesibility() {

		EnableAccessibility(true);
	}

	public void CustomTurnOffAccesibility()
	{
	...
	public bool DecrementContainer(bool resetToStartItem = false)
	{
		if (!HandleUI())
			return false;
	...
	public bool IncrementContainer(bool resetToStartItem = false)
	{
		if (!HandleUI())
			return false;
	...
	public void setPitch(float pitch) {
		customTTSTransform = GameObject.Find("Custom TTS").transform;
		customTTSTransform.GetComponent<AudioSource>().pitch = pitch;
		customTTSTransform.GetComponent<AudioSource>().volume = 0.2f;
	}
	...
	public void Saysomething(string textToSay)
	{
		...
		if (customTTSTransform != null)
		{
			customTTSTransform.GetComponent<AudioSource>().spatialBlend = 0;
		}
	...
	public GameObject getCurrentElement() {
		...
		return m_CurrentItem.m_Object.gameObject;
	}


	public Transform customTTSTransform = null;
	public void Saysomething3D(string textToSay, Transform target)
	{
		if (customTTSTransform == null) {
		...
		if (customTTSTransform != null)
        {
			customTTSTransform.GetComponent<AudioSource>().spatialBlend = 1;
			customTTSTransform.position = target.position;
            customTTSTransform.GetComponent<AudioSource>().minDistance = 275f;
            customTTSTransform.GetComponent<AudioSource>().maxDistance = 500f;
    ...
    public void SaySomethingSound(string textToSay, Transform target, AudioClip mySoundEffect)
    {
		if (customTTSTransform == null)
		{
		...
		if (customTTSTransform != null)
		{
			...
			/*
			m_AudioQueue.GetComponent<AudioSource>().minDistance = 200;
            m_AudioQueue.GetComponent<AudioSource>().maxDistance = 100;
			*/
            m_AudioQueue.GetComponent<AudioSource>().spatialBlend = 1;
			m_AudioQueue.transform.position = target.position;
		}


		UAP_AudioQueue.EAudioType sayAs = UAP_AudioQueue.EAudioType.Element_Text;

		instance.m_AudioQueue.QueueAudio(textToSay, sayAs, true, null);
		instance.m_AudioQueue.QueueAudio(mySoundEffect, UAP_AudioQueue.EAudioType.Element_Text);
		//instance.m_AudioQueue.QueuePause(1f);
		
		//instance.m_AudioQueue.QueueAudio("chair", sayAs, true, null);
