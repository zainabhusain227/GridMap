...
public class UAP_AudioQueue : MonoBehaviour
{
	...
	{
		//tempAudioClip = newEntry.m_Audio;
		...
		if (interrupts != EInterrupt.None)
		{
			...
			{
				//Debug.Log("type of audio"+ m_ActiveEntry.m_AudioType);
				//Debug.Log("Audio file name "+ m_ActiveEntry.m_Audio.name);
				if (((int)m_ActiveEntry.m_AudioType & (int)interrupts) > 0)
				{
					//tempAudioClip = m_ActiveEntry.m_Audio;
